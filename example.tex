...
这是一大段文字这是一大段文字 中英文混合 Numerical Methods. 这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字.
...
第二章, 首先介绍了数学公式的使用, 介绍了定理环境, 给出了定理、命题、引理、推论、证明以及注的环境示例.

第三章, 介绍了微分方程的数值方法中差分方法的示例.

第四章, 介绍了表格环境, 给出了三线表的介绍和自定义表格环境 generaltab 的使用, 也给出了其他表格插入示例.

第五章, 介绍了可以插入各种格式的图片, 然后给出了自定义插图环境 generalfig 和 并排插图实例.
...
数学公式的使用请参考公式 symbols-a4, 或者 《一份(不太)简短的 \LaTeX 2$\varepsilon$ 介绍》 (lshort-zh-cn).
...
请注意 generalfig 第一个参数是标题, 第二个参数是引用.

两个图左右并排放置, 共用一个标题.
\begin{figure}[htp!]
\centering
  \includegraphics[width=0.45\linewidth]{Image1.eps} %2.3in
  \hfill
...
通过 \verb|ref| 引用图片: 引用该图片例如: \autoref{fig:data}.
...
\centering
  \includegraphics[width=0.9\linewidth]{SMError.eps}
    \caption{谱方法的误差.}
    \label{fig:SMError}
\end{minipage}
  \hfill
\begin{minipage}[t]{0.48\linewidth}
\centering
   \includegraphics[width=0.9\linewidth]{CMError.eps}
   \caption{配置方法的误差.}
   \label{fig:CMError}
\end{minipage}
\end{figure}
...
文献引用 \cite{Adams1975, Tadmor2012}和 \cite{LiLiu1997}.
